...
class TestAgree(unittest.TestCase):
    ...
    def setUp(self):
        # 获取登录页面对象
        self.request = PageAgree(GetDriver().get_driver())
    ...
    def test_agree(self, slsl):
        # 调用测试方法
        save_time = self.request.page_agree(slsl)
        sleep(3)
        # 获取断言
        text_time = self.request.page_agree_pass()
        ...
        try:
            ...
            try:
                ...
                self.request.page_agree_assertionview(data.join_path, DataAgree().successname)
                print('审核申领单成功！！！')
            except AssertionError:
                self.request.page_agree_assertionview(data.join_path, DataAgree().errorname)
                print('审核申领单失败！！！')
        except AssertionError:
            self.request.page_agree_assertionview(data.join_path, DataAgree().errorname)
            print('审核申领单失败！！！')
